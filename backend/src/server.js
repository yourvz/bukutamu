...
app.use(
  cors({
    origin: [
      ...
      "http://localhost:5175",
      "http://127.0.0.1:5175",
      process.env.FRONTEND_URL || "http://localhost:5174", // Production frontend URL from .env
      "https://yourvz.github.io", // Add your GitHub Pages URL if needed
    ], // Frontend URLs
...
const validateTamuEntry = (req, res, next) => {
  ...
  if (!telepon || telepon.trim().length < 8) {
    res
      .status(400)
      .json({ error: "Nomor telepon harus diisi minimal 8 karakter" });
    return;
  }
  ...
  if (
    dari !== "umum" &&
    (!req.body.nama_instansi || req.body.nama_instansi.trim().length < 2)
  ) {
...
app.get("/api/tamu", async (_req, res) => {
  try {
    ...
    if (error) {
      ...
      res
        .status(500)
        .json({ error: "Gagal mengambil data", details: error.message });
      return;
    }
...
app.get("/api/tamu/statistics", async (_req, res) => {
  try {
    ...
    if (error) {
      ...
      res
        .status(500)
        .json({ error: "Gagal mengambil statistik", details: error.message });
      return;
    }
